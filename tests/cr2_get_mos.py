"""
  ...
  for Cr2 using a scan technique for getting the correct orbitals
  in the active space.
"""
...
def save_tensors(b, h0, h1, h2, folder="tensors"):
    ...
    filename = os.path.join(folder, f"cr2_{b:2.1f}_tensors.npz")

    # Save arrays; h0 is stored as a 0-d array
    np.savez_compressed(filename, h0=h0, h1=h1, h2=h2)
